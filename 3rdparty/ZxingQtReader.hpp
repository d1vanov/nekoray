...
inline QList<Result> ReadBarcodes(const QImage& img, const ReaderOptions& options = {})
{
	...
	auto exec = [&](const QImage& img) {
		return QListResults(ZXing::ReadBarcodes(
			{img.bits(), img.width(), img.height(), ImgFmtFromQImg(img), static_cast<int>(img.bytesPerLine())}, options));
	};

	return ImgFmtFromQImg(img) == ImageFormat::None ? exec(img.convertToFormat(QImage::Format_Grayscale8)) : exec(img);
}

inline Result ReadBarcode(const QImage& img, const ReaderOptions& options = {})
{
	auto res = ReadBarcodes(img, ReaderOptions{options}.setMaxNumberOfSymbols(1));
	return !res.isEmpty() ? res.takeFirst() : Result();
}

#ifdef QT_MULTIMEDIA_LIB
inline QList<Result> ReadBarcodes(const QVideoFrame& frame, const ReaderOptions& options = {})
{
	using namespace ZXing;
...
#define FIRST_PLANE 0
	...
	if (fmt != ImageFormat::None) {
		...
		return QListResults(ZXing::ReadBarcodes(
			{img.bits(FIRST_PLANE) + pixOffset, img.width(), img.height(), fmt, img.bytesPerLine(FIRST_PLANE), pixStride}, options));
	}
	else {
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
		...
		if (qimg.format() != QImage::Format_Invalid)
			return ReadBarcodes(qimg, options);
		qWarning() << "failed to convert QVideoFrame to QImage";
		return {};
	}
}

inline Result ReadBarcode(const QVideoFrame& frame, const ReaderOptions& options = {})
{
	auto res = ReadBarcodes(frame, ReaderOptions{options}.setMaxNumberOfSymbols(1));
	return !res.isEmpty() ? res.takeFirst() : Result();
}
